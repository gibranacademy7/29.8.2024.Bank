...
# דיקשנרי (מילון) שבו כל חשבון בנק מיוצג על ידי מספר חשבון (מפתח). כל חשבון כולל פרטים
bank_accounts = {
    1001: {
        "first_name": "Alice",
        "last_name": "Smith",
...
# פונקציה זו יוצרת עסקה (מזוכה) מחשבון מקור לחשבון יעד
# היא מוסיפה את פרטי העסקה (כולל תאריך) לרשימה של עסקאות להוצאה בחשבון המקורי
...
# פונקציה זו מבצעת את העסקאות בחשבון
# היא מעדכנת את יתרת החשבון המקורי והחשבון היעד לפי סכום העסקה
# לאחר ביצוע העסקה, היא מוסיפה את פרטי העסקה להיסטוריה של החשבון המקורי ומנקה את רשימת העסקאות להוצאה
...
# פונקציה זו מאפשרת למשתמש להוסיף עסקה חדשה
# היא שואלת את המשתמש על חשבון המקור, חשבון היעד וסכום העסקה, ומוודאת שהנתונים תקינים
def add_trx():
    while True:
        try:
...
# פונקציות אלו עדיין לא ממומשות, אך כנראה מתוכננות לבצע פעולות נוספות כמו ביצוע עסקאות או הצגת דוחות
def execute_trx():
    pass
...
# ונקציה זו מציגה תפריט ראשי למשתמש, שבו הוא יכול לבחור בין אפשרויות שונות (ליצור עסקה, לבצע עסקה, לראות דוחות, או לצאת מהתוכנית)
# היא משתמשת ב- match כדי לזהות את הבחירה של המשתמש ולקרוא לפונקציה המתאימה
def show_main_menu():
    while True:
        print('1. Create trx');
        print('2. Execute trx');
